...
impl JsExecutor {
    ...
    pub fn execute_js(&self, compiled_code: &str) -> Result<String> {
        ...
        let js_runtime = include_str!("../../site/js_runtime.js");
        let js_code = format!(
            "{}\n{}\nexecute(String.raw`{}`);",
            js_runtime,
            r#"
function execute(compiledCode) {
    try {
        ...
        if let Some(124) = output.status.code() {
            // Cache results even on timeout so we can test infinite loops.
            let err_msg = "Execution timeout exceeded (10 seconds)";
            let full_err = format!("JS execution error: {}", err_msg);
            self.cache_result(compiled_code, &full_err)?;
